...
class MatchDecks {
    ...
    static async playDeckCard(game,deckId) {
        try {
            ...
            if ((card.cardId == 6 || card.cardId == 7) && playerObj.state.name != "Ready") {
            ...
            switch (card.cardId) {
                case 1: attackFullSalvo(oppObj); break;
                case 2: attackAimedShot(oppObj); break;
                case 3: attackBarrage(oppObj); break;
                case 4: await attackWeakSpot(game.player.id,oppObj); break;
                case 5: healRepair(playerObj); break;
                case 6: healLastHope(playerObj); break;
                case 7: defenseManeuvers(playerObj); break;
            }
...
function attackFullSalvo(oppObj) {
    if (oppObj.state.name!="Defensive") {
        oppObj.hp -= 3;
    }
}
function attackAimedShot(oppObj) {
    if (oppObj.state.name!="Defensive") {
        oppObj.hp -= (oppObj.hp < Settings.halfHP)?2:1;
    }
}
function attackBarrage(oppObj) {
    oppObj.hp -= 2;
}
async function attackWeakSpot(playerId,oppObj) {
    if (oppObj.state.name!="Defensive") {
...
function healRepair(playerObj) {
    playerObj.hp += 2;
    if (playerObj.hp > Settings.maxShipHP) playerObj.hp = Settings.maxShipHP;
}

function healLastHope(playerObj) {
    playerObj.hp += 4;
    if (playerObj.hp > Settings.maxShipHP) playerObj.hp = Settings.maxShipHP;
    else if (playerObj.hp < Settings.halfHP) playerObj.hp = Settings.halfHP;
    // Action points go to zero
    playerObj.ap = 0;
}

function defenseManeuvers(playerObj) {
    playerObj.state.id = 3;
    // we will only use the id to update the DB, so this line shouldn't be needed 
    playerObj.state.name = "Defensive";
}
